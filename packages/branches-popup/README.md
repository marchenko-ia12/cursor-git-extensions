...
- **Smart Checkout** — if checking out a branch would overwrite your uncommitted changes, a dialog offers **Smart Checkout** (stash → switch → pop), **Force Checkout** (discard), or **Don't Checkout** — the JetBrains affordance, no more `error: Your local changes ... would be overwritten by checkout`.
